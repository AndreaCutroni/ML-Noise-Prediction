...
### The 11 configurations (per task, per model)

`full` (all 18) · **leave-one-out** `−landuse −centrality −points −distances −core` · **single-group-only** `only-landuse only-centrality only-points only-distances only-core`.
...
*All numbers come from `results/ablation_{classification,regression}_results.csv` (33 rows each = 11
